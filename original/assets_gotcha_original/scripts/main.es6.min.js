...
function initialize() {
	...
	var customerRetrievesOrder = document.getElementById("customerRetrievesOrder");
	var deliveryAddress = document.getElementById("deliveryAddress");

	// Change event listeners
	document.addEventListener("change", function (event) {
		if (event.target.classList.contains("file-upload__image-upload") || event.target.classList.contains("file-upload__sketch-upload")) {
	...
	document.addEventListener("click", function (event) {
		...
		// Mobile side navigation
		if (event.target.id === 'sidenavigationBtn' || event.target.closest('#sidenavigationBtn') !== null) {
			sidenavigationBtn.classList.toggle('js-active');
			sidenavigation.classList.toggle('js-active');
		}

		// Accordion headers
		if (event.target.classList.contains('accordion__expand') || event.target.closest('.accordion__expand') !== null) {
			event.target.closest('.accordion').classList.toggle('js-active');
		}

		// File upload
		if (event.target.classList.contains("file-upload__thumbnail") || event.target.classList.contains("file-upload__thumbnail--container")) {
			var imageThumbnail = void 0;
			var uploadContainer = event.target.closest(".file-upload--container");
			event.target.classList.contains("file-upload__thumbnail") ? imageThumbnail = event.target : imageThumbnail = event.target.children[0];
			imageThumbnail.src = "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7";
		...
		if (event.target.classList.contains('btn--tab-navigation')) {
			...
			if (event.target.classList.contains('btn--prev')) {
				...
				if (previousTabContainer !== undefined) {
					...
					document.querySelector('.tab-navigation__item.js-active').classList.remove('js-active');
					document.querySelector('[data-tab=' + previousTabContainer.id + ']').classList.add('js-active');
				}
			}
			// Next
			if (event.target.classList.contains('btn--next')) {
				...
				if (nextTabContainer !== undefined) {
					...
					document.querySelector('.tab-navigation__item.js-active').classList.remove('js-active');
					document.querySelector('[data-tab=' + nextTabContainer.id + ']').classList.add('js-active');
				}
			}
		}
		...
		if (event.target.classList.contains('tab-navigation__item')) {
			var currentTab = document.querySelector('.tab-navigation__item.js-active');
			var currentTabContainer = document.getElementById(currentTab.getAttribute('data-tab'));
			var newTabContainer = document.getElementById(event.target.getAttribute('data-tab'));
		...
		if (event.target.classList.contains('document-list__link--expand') || event.target.closest('.document-list__link--expand')) {
			event.target.closest('.document-list__file--sub').classList.toggle('js-active');
		}

		// Document sublist expand button
		if (event.target.classList.contains('document-list__link--sub-expand') || event.target.closest('.document-list__link--sub-expand')) {
			event.target.closest('.document-list__file--sub-list').classList.toggle('js-active');
		}
	});

	// Input clear button
	document.addEventListener("focus", function (event) {
		if (event.target.tagName === "INPUT" && event.target.type === "text" && event.target.readOnly === false) {
			inputClearElement.style.opacity = "1";
			event.target.parentElement.appendChild(inputClearElement);
		}
		if (event.target.parentElement.classList.contains('product-list--container')) {
	...
	function fileUploadEventListener(input) {
		input.addEventListener('change', function (e) {
			var thumbnail = input.parentElement.querySelector('.file-upload__thumbnail');;
